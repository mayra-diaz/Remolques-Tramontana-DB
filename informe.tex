...
\begin{itemize}
    \item Cuántas horas trabajaron por máquina los 10 empleados de mayor sueldo del área de fabricación.
    \item Todos los bienes que sean de la categoría 'Construcción', que cuenten con stock (mayor a 0) y hayan sido compradas en X año por un precio unitario mayor a 10 soles.
    \item Máquinas que hayan utilizado bienes medidos en kilogramos por una cantidad total mayor a 12 durante el año X.
\end{itemize}
...
\begin{enumerate}
    \item Crear una base de datos con cuatro esquemas a partir del mismo modelo relacional que se puede observar en el \textbf{Anexo B}, que tengan 1K, 10K, 100K y 1M de tuplas, respectivamente. Utilizar el \textbf{"MAYRA PON COMO CREASTE LA WEA ACA"} para generar los datos.
    \item Se crean índices para cada consulta para poder hacer un análisis posterior de las consultas con y sin índices.
    \item Se procede a desactivar la utilización automática de índices por parte de PosgreSQL. Asimismo, se desactivan los índices creados. En la Consulta 1 se dejan los índices default activados para hacer una comparación posterior del tiempo de ejecución. Para lograrlo se ejecutan los siguientes comandos:
...
Para la primera consulta 1 se puede apreciar en la figura 3 que no existe
una diferencia apreciable entre las consultas con ´ındices y sin ´ındices cuando
el tama˜no del dataset es menor 10k, a partir de 10k en adelante ya se puede
observar una diferencia que va aumentando en relaci´on al tama˜no del dataset.
...
Para esta segunda consulta a diferencia de la consulta anterior solo se puede
apreciar una ligera diferencia cuando el tama˜no del dataset es de 100k.
...
Para esta consulta al igual que en las consultas anteriores no hay una diferencia apreciable cuando el tama˜no del dataset es menor a 10k y s´ı cuando pasa
este tama˜no como se puede ver para cuando el tama˜no es de 100k y mucho m´as
a´un en 1000k.
...
\textbf{Con Índice}   & 1.31          & 13.836         & 99.258          & 625.764      \\
\textbf{Consulta 3}   & 3.5797        & 39.842         & 285.893         & 1550.183     \\ \hline
\end{tabular}
